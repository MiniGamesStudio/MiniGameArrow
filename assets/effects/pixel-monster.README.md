...
用于 Cocos Creator 3.8 的 32x32 程序化像素怪物 shader，主色从 `assets/resources/palettes/monster_palette.png` 这张色板图里采样。

## 使用方式

1. 在场景里创建一个带 `Sprite` 的节点。
2. 添加 `PixelMonsterSprite` 组件。
3. 可以不手动拖资源：组件会自动创建 32x32 SpriteFrame 载体，并从 `resources/effects/pixel-monster`、`resources/palettes/monster_palette/texture` 加载默认 shader 和色板。
