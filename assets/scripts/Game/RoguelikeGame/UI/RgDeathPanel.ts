...
/**
 * 死亡结算面板
 * 显示存活时间、击杀数、获得金币等统计数据
 */
@ccclass('RgDeathPanel')
export class RgDeathPanel extends UIBase {
    /** 存活时间文本 */
    @property(Label)
    ...
    /** 到达楼层文本 */
    @property(Label)
    m_FloorReachedLabel: Label = null;
    ...
    private _refreshStats(result?: RunResult, settlement?: RunSettlement): void {
        ...
        if (this.m_FloorReachedLabel) {
            this.m_FloorReachedLabel.string = `${result?.floorReached ?? 1}F`;
        }
