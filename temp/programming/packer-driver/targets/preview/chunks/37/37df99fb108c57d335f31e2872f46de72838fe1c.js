    ...
    execute: function () {
      ...
      _export("Flower", Flower = (_dec = ccclass('Flower'), _dec(_class = class Flower extends Component {
        ...
        tryFindEmptySlot(selfCollider, otherCollider) {
          ...
          if (!light) return;
          var detectedPos = this.detectImgPosition(selfCollider, otherCollider);
          var priority = (_crd && SLOT_PRIORITY === void 0 ? (_reportPossibleCrUseOfSLOT_PRIORITY({
            error: Error()
          }), SLOT_PRIORITY) : SLOT_PRIORITY)[detectedPos];

          for (var pos of priority) {
            var slotName = (_crd && SLOT_NAMES === void 0 ? (_reportPossibleCrUseOfSLOT_NAMES({
              error: Error()
            }), SLOT_NAMES) : SLOT_NAMES)[pos];
            var slot = light.getChildByName(slotName);
        ...
        detectImgPosition(selfCollider, otherCollider) {
          ...
          var threshold = otherCollider.worldAABB.size.width / 6;
          var deltaX = selfCollider.node.getWorldPosition().x - otherCollider.node.getWorldPosition().x;
        ...
        startDrag(event) {
          ...
          var localPos = this.m_FlowerMoveRootUIT.convertToNodeSpaceAR(new Vec3(touchPos.x, touchPos.y, 0));
        ...
        animateFlowerBack(target, prevTag) {
          var endPos = target.getWorldPosition();
